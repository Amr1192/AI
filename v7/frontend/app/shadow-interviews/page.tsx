...
export default function ShadowInterviewPage() {
  ...
  function startEvents(id: number, sid: string) {
    ...
    es.onopen = () => console.log("✅ SSE connected for", sid);
  ...
  async function startRecording(id: number, sid: string) {
    ...
    mediaStreamRef.current = full;
    if (videoRef.current && isVideoOn) videoRef.current.srcObject = full;

    const audioOnly = new MediaStream(full.getAudioTracks());
  ...
  async function stopRecording() {
    ...
    // stop local media now
    hardStopMedia();

    // close server-side session cleanly if present
  ...
  async function startInterview() {
    ...
    const qRes = await fetch(
      `${API_BASE}/api/interviews/${startData.id}/next-question`
    );
    ...
    // start RT session (one call)
    const sRes = await fetch(
      `${API_BASE}/api/interviews/${startData.id}/rt/start`,
      { method: "POST" }
    );
  ...
  async function nextQuestion() {
    ...
    // stop current question cleanly
    await stopRecording();

    // cool down a bit (release devices)
    await wait(800);
    ...
    // start a **fresh** RT session (single call) for this question
    const sRes = await fetch(
    ...
    // open SSE first so partials are caught as soon as chunks arrive
    startEvents(interviewId, newSid);
    await wait(150);

    // record using the **new** session id
    startRecording(interviewId, newSid);
              ...
              {feedback.length > 0 ? (
                feedback.map((p, i) => (
                  <div key={i} className="text-sm border-b pb-2">
                    ...
                    <div>
                      <b>Filler:</b> {p.fillerWords ?? 0} | <b>Pace:</b>{" "}
                      {p.pace ?? "good"}
                    </div>
                  </div>
                ))
